...
class IsaacGym(BaseSimulator):
    ...
    def _build_each_env(self, env_id, env_ptr):
        ...
        if self.simulator_config.sim.physx.get('enable_dof_force_sensors', False):
    ...
    def _process_rigid_body_props(self, props, env_id):
        ...
        if self.env_config.domain_rand.randomize_base_com:
            ...
            torso_index = self._body_list.index(self.env_config.robot.get('torso_name', 'torso_link'))
        ...
        if self.env_config.domain_rand.randomize_base_mass:
            ...
            base_index = self._body_list.index(self.env_config.robot.get('base_name', 'pelvis')) # for fixed upper URDF we only have pelvis link
            assert base_index != -1
