...
class LeggedRobotDecoupledLocomotionStance(LeggedRobotLocomotion):
    def __init__(self, config, device):
        ...
        self.action_scale_upper_body = torch.zeros(self.num_envs, 1, dtype=torch.float, device=self.device, requires_grad=False)
    ...
    def _reward_penalty_stance_dof(self):
        ...
        return torch.sum(torch.abs(self.simulator._rigid_body_vel[:, self.pelvis_id]), dim=1) * (1.0 - self.commands[:, 4])
        # Penalize dof pos change of the lower body
        return torch.sum(torch.square(self.last_dof_pos[:, self.lower_dof_indices] - \
                                      self.simulator.dof_pos[:, self.lower_dof_indices]), dim=1) * (1.0 - self.commands[:, 4])
