...
class BaseTask():
    ...
    def _init_buffers(self):
        ...
        for key in self.config.obs.obs_dict:
            ...
            else: self.history_length[key] = 1
        self.obs_buf_dict = {}
        for key in self.config.obs.obs_dict:
            self.obs_buf_dict[key] = torch.zeros(self.num_envs, 
    ...
    def _setup_robot_body_indices(self):
        ...
        for i in range(len(penalized_contact_names)):
            ...
            self.penalized_contact_indices[i] = self.simulator.find_rigid_body_indice(penalized_contact_names[i])
